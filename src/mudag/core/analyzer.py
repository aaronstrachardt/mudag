...
def is_workflow_file(file_path: str) -> bool:
    ...
    workflow_languages = {
        ...
        "Snakemake": ["Snakefile", "snakefile", "SNAKEFILE", "Snake", "snake", ".smk", ".snake", ".snakefile", ".snakemake", ".rules", ".rule"],
    ...
    if re.match(r"snakefile[\._-]\d+", basename_lower) or re.match(r"snake[\._-]\d+", basename_lower):
        return True
...
def count_lines(file_path: str) -> Dict[str, int]:
    ...
    if ext in ['.py', '.smk', '.snake', '.snakefile', '.snakemake', '.rules', '.rule'] or \
       basename_lower in ['snakefile', 'snake'] or \
       basename_lower.startswith('snakefile') or basename_lower.startswith('snake'):
        ...
        block_starts = ['/*']
        block_ends = ['*/']
    elif ext in ['.knwf', '.workflow.knime', '.knar'] or basename_lower.endswith('.workflow.knime'):
        # KNIME (XML-based)
        line_comment = '<!--'
        block_starts = ['<!--']
        block_ends = ['-->']
                ...
                if block_starts:
                    ...
                    for j, block_start in enumerate(block_starts):
                        if line.strip().startswith(block_start):
                            comment_lines += 1
                            if block_ends[j] not in line[line.find(block_start) + len(block_start):]:
...
def get_workflow_language(file_path: str) -> str:
    ...
    # Check for Snakemake
    if ext in ['.smk', '.snake', '.snakefile', '.snakemake', '.rules', '.rule'] or \
       basename_lower in ['snakefile', 'snake'] or \
       basename_lower.startswith('snakefile') or basename_lower.startswith('snake'):
        return "Snakemake"
...
def scan_directory(
    ...
    for root, dirs, files in os.walk(directory):
        ...
        dirs[:] = [d for d in dirs if not ignore_patterns.is_ignored(os.path.join(root, d))]
        ...
        for file in files:
            ...
            results['__metadata__']['workflow_languages'][language]['code'] += line_counts['code']
            results['__metadata__']['workflow_languages'][language]['comment'] += line_counts['comment']
            results['__metadata__']['workflow_languages'][language]['blank'] += line_counts['blank']
            results['__metadata__']['workflow_languages'][language]['total'] += line_counts['total']
