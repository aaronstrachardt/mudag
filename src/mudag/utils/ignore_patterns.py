    ...
    def is_ignored(self, path: str) -> bool:
        ...
        for pattern in self.patterns:
            ...
            if pattern.endswith('/'):
                ...
                if (path == dir_name or  # Exact match
                    path.startswith(f"{dir_name}/") or  # Path is inside this directory directly
                    f"/{dir_name}/" in f"/{path}/" or  # Path contains this directory
                    path.endswith(f"/{dir_name}")):  # Path ends with this directory
                    return True
            else:
