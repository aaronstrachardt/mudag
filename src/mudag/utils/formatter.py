...
def format_table(results: Dict[str, Dict[str, int]], output: TextIO) -> None:
    ...
    path_width = max(len(os.path.relpath(path)) for path in results.keys()) + 2 if results else 10
    ...
    for idx, (path, counts) in enumerate(sorted(results.items()), 1):
        ...
        output.write(f"{idx:<{idx_width}} | {rel_path:<{path_width}} | {code:<8} | {comment:<8} | {blank:<8} | {total:<8}\n")
    
    # Print separator
    output.write(f"{separator}\n")
    
    # Print overall total
    output.write(f"{'TOTAL':<{idx_width + path_width + 3}} | {total_code:<8} | {total_comment:<8} | {total_blank:<8} | {total_lines:<8}\n")
    ...
    if metadata and 'workflow_languages' in metadata:
        ...
        if total_files > 0:
            ...
            output.write(f"{'-' * 40}\n")
            output.write(f"{'Language':<15} | {'Files':<8} | {'Code':<8} | {'Comment':<8} | {'Blank':<8} | {'Total':<8}\n")
            output.write(f"{'-' * 70}\n")
            
            # Print statistics for each language that has files
            for lang, stats in sorted(languages.items()):
                if stats['files'] > 0:
                    output.write(f"{lang:<15} | {stats['files']:<8} | {stats['code']:<8} | {stats['comment']:<8} | {stats['blank']:<8} | {stats['total']:<8}\n")
...
def format_json(results: Dict[str, Dict[str, int]], output: TextIO) -> None:
    ...
    if metadata and 'workflow_languages' in metadata:
        ...
        language_stats = {lang: stats for lang, stats in metadata['workflow_languages'].items() if stats['files'] > 0}
        if language_stats:
...
def format_csv(results: Dict[str, Dict[str, int]], output: TextIO) -> None:
    ...
    # Write header
    writer.writerow(['File Path', 'Code Lines', 'Comment Lines', 'Blank Lines', 'Total Lines'])
    ...
    if metadata and 'workflow_languages' in metadata:
        ...
        if len(results) > 0:
            ...
            writer.writerow(['Workflow Language', 'Files', 'Code Lines', 'Comment Lines', 'Blank Lines', 'Total Lines'])
            
            # Add data for each language that has files
            for lang, stats in sorted(languages.items()):
                if stats['files'] > 0:
                    writer.writerow([lang, stats['files'], stats['code'], stats['comment'], stats['blank'], stats['total']]) 